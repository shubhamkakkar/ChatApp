...
export default class LoginSignupScreen extends React.Component {
  ...
  axiosCall = credentials =>
    axios
      .post(`http://${systemIP}:${backendPort}/user`, credentials)
      .then(res => {
        if (res.status === 200) {
          const { getUserId, navigation } = this.props;
          const idProp = res.data.user._id;
          getUserId(idProp);
          navigation("User", { idProp });
        } else {
          alert("Error");
        }
      })
  ...
  loginSubmit = credentials => {
    ...
    if (validationCheck) {
      ...
      if (name.length && email.length && password.length) {
        const newCredential = { name, email };
        this.axiosCall(newCredential);
